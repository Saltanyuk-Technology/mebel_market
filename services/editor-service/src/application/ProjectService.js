import { migrateFurnitureDocument } from "../persistence/migrations.js";
...
export class ProjectService {
  ...
  toDocument(workspace, metadata = {}) {
    const next = migrateFurnitureDocument(workspace, metadata);
  ...
  async save({ definitionId, basedOnRevisionId, name, componentType = "cabinet", workspace, kitchenProjectId }) {
    const document = this.toDocument(workspace, { name, componentType });
    const body = { name, componentType, document, placementMetadata: document.placement, kitchenProjectId };
