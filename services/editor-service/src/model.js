...
const MIN_CONFIRMAT_SPACING_MM = 100;
const PART_TYPES = new Set(["bottom", "side", "shelf", "top"]);
const PANEL_MATERIALS = {
  "ldsp-16": { name: "ЛДСП 16 мм", thicknessMm: 16 },
  "hdf-4": { name: "ХДФ 4 мм", thicknessMm: 4 },
};

function panelMaterial(material) {
  return PANEL_MATERIALS[material] ?? PANEL_MATERIALS["ldsp-16"];
}
...
export function constrainMeasurementPoint(start, end) {
  if (!start || !end) return end;
  const deltas = {
    xMm: Math.abs(end.xMm - start.xMm),
    yMm: Math.abs(end.yMm - start.yMm),
    zMm: Math.abs(end.zMm - start.zMm),
  };
  const axis = Object.entries(deltas).sort((first, second) => second[1] - first[1])[0][0];
  return {
    xMm: axis === "xMm" ? end.xMm : start.xMm,
    yMm: axis === "yMm" ? end.yMm : start.yMm,
    zMm: axis === "zMm" ? end.zMm : start.zMm,
  };
}
...
export function getConfirmatPlacement(model, connectionOrIds, insetOverride) {
  ...
  const availableSpacing = spanLength - effectiveInset * 2;
...
export class FurnitureModel {
  ...
  addCustomPart({
    ...
    const resolvedMaterial = Object.hasOwn(PANEL_MATERIALS, material) ? material : "ldsp-16";
    const materialDefinition = panelMaterial(resolvedMaterial);
    const resolvedPartType = resolvedMaterial === "hdf-4"
      ? null
      : PART_TYPES.has(partType) ? partType : "shelf";
    const typeNames = {
      bottom: "Дно",
      side: "Боковина",
      shelf: "Полка",
  ...
  updatePart(id, changes) {
    ...
    part.partType = part.material === "hdf-4"
      ? null
      : PART_TYPES.has(changes.partType) ? changes.partType : (part.partType ?? "shelf");
    ...
    part.sizeY = isLeg
      ? 100
      : panelMaterial(part.material).thicknessMm;
  ...
  movePartConstrained(id, changes) {
    ...
    if (part.hardwareType !== "leg") {
      ...
      const candidate = {
        ...
        sizeY: panelMaterial(part.material).thicknessMm,
      ...
      if (positionChanged) {
        ...
        movingAxes.forEach((axis) => {
          ...
          this.parts.filter((other) => other.id !== id && other.kind !== "hardware").forEach((other) => {
            ...
            const touchesOnAnotherAxis = definition.others.some(([min, max]) => (
              Math.abs(candidateBounds[min] - otherBounds[max]) <= 1.01
              || Math.abs(candidateBounds[max] - otherBounds[min]) <= 1.01
            ));
            // Once two panels already meet on another axis (for example a side
            // standing on a bottom), align their matching outer faces first.
            // Choosing an external contact here offsets the panel by its own
            // thickness instead of producing a flush furniture corner.
            const options = touchesOnAnotherAxis && aligned.length
              ? aligned
              : external.length ? external : aligned;
  ...
  moveConfirmat(connectionId, pointIndex, positionMm) {
    ...
    if (!connection || !Number.isInteger(index) || index < 0) return false;
  ...
  restore(state) {
    ...
    this.parts.forEach((part) => {
      if (part.kind !== "hardware") {
        const resolvedMaterial = Object.hasOwn(PANEL_MATERIALS, part.material) ? part.material : "ldsp-16";
        const materialDefinition = panelMaterial(resolvedMaterial);
        part.material = resolvedMaterial;
        part.materialName = materialDefinition.name;
        part.sizeY = materialDefinition.thicknessMm;
        part.partType = resolvedMaterial === "hdf-4"
          ? null
          : PART_TYPES.has(part.partType) ? part.partType : "shelf";
      }
      if (part.hardwareType === "leg") {
        part.sizeX = 57;
        part.sizeY = 100;
