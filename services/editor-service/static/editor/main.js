...
let workspaceProjectId = editorUrlParams.get("projectId");
const libraryItemId = editorUrlParams.get("libraryItem");
let currentProjectName = "";
let draftId = editorUrlParams.get("draft");
if (editorUrlParams.get("new") === "1") {
  draftId = crypto.randomUUID();
  const params = new URLSearchParams({ draft: draftId });
  if (workspaceProjectId) params.set("projectId", workspaceProjectId);
...
saveProjectForm.addEventListener("submit", async (event) => {
  ...
  if (libraryItemId && !window.confirm("Сохранение создаст новую версию библиотечного оригинала. Продолжить?")) return;
  ...
  try {
    ...
    if (workspaceProjectId) params.set("projectId", workspaceProjectId);
    if (libraryItemId) params.set("libraryItem", libraryItemId);
    window.history.replaceState({}, "", `${location.pathname}?${params}`);
    byId("save-project").classList.add("saved");
    byId("save-project").textContent = libraryItemId ? "Обновить библиотеку" : "Сохранено";
  ...
  try {
    if (!currentProjectId) {
      const saved = await projectService.save({
        ...
        workspace: workspaceData(), projectId: workspaceProjectId,
      });
...
function queueServerAutosave() {
  ...
  serverAutosaveTimer = setTimeout(() => {
    ...
    byId("save-project").textContent = libraryItemId ? "Обновить библиотеку" : currentProjectId ? "Есть изменения" : "Сохранить";
  }, 2000);
}
...
if (libraryItemId) {
  byId("save-project").textContent = "Обновить библиотеку";
  byId("save-project").title = "Создать новую версию библиотечного оригинала";
}
