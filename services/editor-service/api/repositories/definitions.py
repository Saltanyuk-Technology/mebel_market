...
class DefinitionRepository:
    ...
    async def create_definition(
        ...
        *,
        ...
        scope: str = "library",
        definition_id=None,
        kitchen_project_id=None,
        legacy_project_id=None,
    ):
        row = await self.orm.fetch_one(
            """INSERT INTO furniture_definitions
                 (id, user_id, kitchen_project_id, name, component_type, scope, legacy_project_id)
               VALUES ($1, $2, $3, $4, $5, $6, $7)
               RETURNING *""",
            definition_id or uuid.uuid4(), user_id, kitchen_project_id,
            str(name).strip()[:160] or "Новый элемент", component_type, scope, legacy_project_id,
        )
        return dict(row)
    ...
    async def list_definitions(self, user_id: int, *, kitchen_project_id=None):
        if kitchen_project_id is None:
            ...
            rows = await self.orm.fetch_all(
                """SELECT * FROM furniture_definitions
                   WHERE user_id = $1 AND kitchen_project_id = $2 ORDER BY updated_at DESC""",
                user_id, kitchen_project_id,
            )
