...
async def list_definitions():
    kitchen_id = request.args.get("kitchenProjectId")
    if kitchen_id and not _uuid(kitchen_id):
        return jsonify(error="invalid_kitchen_project_id"), 400
    rows = await DefinitionRepository(editor_orm).list_definitions(
        g.company.user_id, kitchen_project_id=_uuid(kitchen_id)
    )
...
async def create_definition():
    ...
    kitchen_id = _uuid(payload.get("kitchenProjectId"))
    if payload.get("kitchenProjectId") and not kitchen_id:
        return jsonify(error="invalid_kitchen_project_id"), 400
    if kitchen_id and not await KitchenRepository(editor_orm).get(g.company.user_id, kitchen_id):
    ...
    definition = await repository.create_definition(
        ...
        scope="kitchen-project" if kitchen_id else payload.get("scope", "library"),
        kitchen_project_id=kitchen_id,
    )
