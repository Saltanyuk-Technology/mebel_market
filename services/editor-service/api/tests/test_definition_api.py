...
class DefinitionApiTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_definition_cannot_attach_to_another_owners_kitchen(self):
        kitchen_id = uuid.uuid4()
        await editor_orm.execute(
            "INSERT INTO kitchen_projects (id, user_id, name) VALUES ($1, $2, 'Чужая кухня')",
            kitchen_id, self.user_id + 1,
        )
        response = await self.client.post("/api/editor/definitions", json={
            "name": "Шкаф", "document": document(), "kitchenProjectId": str(kitchen_id),
        })
