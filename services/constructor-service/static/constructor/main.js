...
const roomId = constructorParams.get("room");
let activeRoom = null;
...
controller.roomEditingEnabled = true;
...
function renderProjectFurniture() {
  ...
  list.replaceChildren(...projectFurniture.map((item) => {
    ...
    button.addEventListener("click", async () => {
      try {
        const placement = await furnitureRepository.createRoomInstance(roomId, item);
        placement.moveTo({ xMm: ownPlacements.length * 700, zMm: 0 });
        await furnitureRepository.saveRoomInstance(roomId, placement);
...
document.getElementById("remove-placement").addEventListener("click", async () => {
  if (selectedPlacementId) await furnitureRepository.deleteRoomInstance(roomId, selectedPlacementId).catch(() => {});
...
async function loadProjectRoom() {
  ...
  loadingProject = true;
  try {
    const loaded = await furnitureRepository.loadRoom(roomId);
    ...
    title.hidden = false;
    document.title = `${activeRoom.name} — Конструктор помещения`;
  } catch {
    ui.showToast("Не удалось открыть помещение проекта");
  } finally {
...
function queueProjectSave() {
  if (!roomId || loadingProject || !activeRoom) return;
  clearTimeout(projectSaveTimer);
  projectSaveTimer = setTimeout(async () => {
    try {
      await Promise.all([
        furnitureRepository.saveRoom(roomId, { name: activeRoom.name, roomData: room.toJSON() }),
        ...placements.map((placement) => furnitureRepository.saveRoomInstance(roomId, placement)),
      ]);
    } catch { ui.showToast("Не удалось сохранить помещение"); }
  }, 800);
}

document.getElementById("refresh-project-furniture").addEventListener("click", loadProjectRoom);
